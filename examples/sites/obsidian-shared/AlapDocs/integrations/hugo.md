---
source: integrations/hugo.md
modified: '2026-04-15T15:42:57Z'
tags:
- integrations
title: hugo-alap
description: Hugo module for Alap multi-target link menus. Provides an `alap` shortcode
  that emits `` web components — the full expression language (tags, macros, operators,
  regex search, protocols,…
---
# hugo-alap
...
Hugo shortcodes are Go templates, not Go code. While Alap has a [[cookbook/language-ports|full Go parser]] (source: `src/other-languages/go/`), Hugo's template engine can't call arbitrary Go functions. A custom Hugo build or a Go template reimplementation of the parser would be fragile and incomplete. The web component approach guarantees full feature access with zero maintenance burden.
