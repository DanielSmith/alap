---
source: getting-started/all-together.md
modified: '2026-04-21T20:15:53Z'
tags:
- getting_started
title: All Together — Lightbox, Lens, and Beyond
description: '**Getting Started:** Installation · Quick Start · Configuration · Bare
  Minimum · With a Framework · **This Page**'
---
# All Together — Lightbox, Lens, and Beyond

**[[getting-started/README|Getting Started]]:** [[getting-started/installation|Installation]] · [[getting-started/quick-start|Quick Start]] · [[getting-started/configuration|Configuration]] · [[getting-started/bare-minimum|Bare Minimum]] · [[getting-started/with-framework|With a Framework]] · **This Page**

Menus are the starting point. A full Alap build adds three more renderers — lightbox, lens, and embed — plus the AT Protocol for live feeds. This guide shows how to wire them up.
...
Give each renderer its own selector so triggers bind to exactly one:

```typescript
import { AlapUI, AlapLightbox, AlapLens, RendererCoordinator } from 'alap';
import { RENDERER_MENU, RENDERER_LIGHTBOX, RENDERER_LENS } from 'alap';
import 'alap/lightbox.css';
import 'alap/lens.css';
...
`bindKeyboard()` attaches a capture-phase Escape handler, so Escape walks the coordinator's stack — `lens → lightbox → menu → closed` — instead of each renderer handling the key on its own.

To push state onto the stack deliberately (e.g. from an in-page button that opens a gallery), call `coordinator.transitionTo()`:

```typescript
document.getElementById('open-gallery')?.addEventListener('click', (e) => {
  const links = menu.getEngine().resolveProgressive('.bridge', undefined).resolved;
  coordinator.transitionTo(RENDERER_LIGHTBOX, {
    links,
...
See the runnable demo in `examples/sites/all-together/` — it wires three triggers (one per renderer) plus three `transitionTo` buttons on a single page.
...
Web component renderers use shadow DOM and `::part()` for styling. See [[framework-guides/web-component|Web Component guide]] for the full parts list.
...
- [[api-reference/lightbox|Lightbox API]] — full options, CSS custom properties, `::part()` selectors
- [[api-reference/lens|Lens API]] — full options, transitions, meta fields
- [[api-reference/embeds|Embeds API]] — providers, consent management
- [[api-reference/coordinators|Coordinators API]] — transitions, back stack, View Transitions
