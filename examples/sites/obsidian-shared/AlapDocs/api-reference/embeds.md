...
- **Iframe hardening.** Iframes render with `referrerpolicy="strict-origin-when-cross-origin"`, `loading="lazy"`, and a locked `allow` attribute scoped to the providers' oembed needs. `sandbox` is intentionally not set — several providers fail in sandboxed iframes; hardening relies on the `allow` policy and the provider allowlist instead. See [[cookbook/embeds#security|Security in the cookbook]] for the full list.

Embeds are protected by Alap's standard URL sanitization. For more on how Alap ensures that data from different sources is handled safely, see our [[api-reference/security#trust-model-how-alap-handles-data|Security Trust Model]].
