...
title: Docusaurus source — example for `--ssg docusaurus`
description: A small Docusaurus 3.x site — reading notes on three 19th-century English
  novels. Body content is adapted from Wikipedia (CC BY-SA 4.0); the Docusaurus syntax
  is mine.
---
...
- `` → stripped
- `` → stripped
- `…` and `…` → outer tags stripped,
  inner content kept
- `## Heading {#anchor}` and `## Heading ` → anchor
