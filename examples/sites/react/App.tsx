...
function OperatorExamples() {
  return (
    <section>
      <h2>Operators</h2>
      <p className="section-note">AND, OR, WITHOUT — same expression language as vanilla.</p>
      <div className="example">
        <p>
    ...
    <section>
      ...
      <div className="example">
        <p>
          <AlapLink
            query=".sf"
            menuStyle={{ borderRadius: '12px', border: '2px solid #10b981' }}
          >
          ...
          <AlapLink
            query=".bridge"
            menuStyle={{ maxWidth: '160px', fontSize: '0.8rem' }}
          >
            bridges (compact)
          </AlapLink>
        </p>
...
function HookExample() {
  ...
  const [expression, setExpression] = useState('.coffee + .sf');
  const ids = query(expression);
  const links = resolve(expression);
  ...
  return (
    <section>
      ...
      <div className="example">
        ...
        <p className="section-note">
          IDs: <code>{ids.join(', ') || '(none)'}</code>
        </p>
        {links.length > 0 && (
          <ul className="hook-results">
            {links.map((link) => (
              <li key={link.id}>
...
function DynamicConfigExample() {
  const [config, setConfig] = useState<AlapConfig>(demoConfig);
      ...
      <section>
        ...
        <div className="example">
          <p>
            ...
            {' '} — {' '}
            <button onClick={addLink} disabled={added} className="demo-button">
        ...
        <pre><code>{`const [config, setConfig] = useState(demoConfig);
...
export function App() {
  return (
    <AlapProvider config={demoConfig}>
      <h1>Alap v3 — React Adapter</h1>
      <p>
        All examples use <code>&lt;AlapProvider&gt;</code> +{' '}
        <code>&lt;AlapLink&gt;</code> from <code>alap/react</code>.
      </p>
