...
export default function App() {
  const [strategy, setStrategy] = createSignal<Strategy>('flip');
  const [barPosition, setBarPosition] = createSignal<'left' | 'right'>('left');

  const p = (compass: string) => `${compass}, ${strategy()}`;
  ...
  return (
    <AlapProvider config={sandboxConfig}>
      ...
      <div class="strategy-picker">
        ...
        <label><input type="radio" name="strategy" checked={strategy() === 'flip'} onChange={() => setStrategy('flip')} /> flip <span class="strategy-hint">(default — flips if it doesn't fit)</span></label>
        <label><input type="radio" name="strategy" checked={strategy() === 'clamp'} onChange={() => setStrategy('clamp')} /> clamp <span class="strategy-hint">(flip + constrain to viewport)</span></label>
        <label><input type="radio" name="strategy" checked={strategy() === 'place'} onChange={() => setStrategy('place')} /> place <span class="strategy-hint">(pinned, no fallback)</span></label>
