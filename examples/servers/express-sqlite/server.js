...
app.get('/configs/:name', (req, res) => {
  const row = stmts.get.get(req.params.name);
...
app.put('/configs/:name', (req, res) => {
  ...
  warnHyphens(config);
  stmts.upsert.run(req.params.name, JSON.stringify(config));
  res.status(204).end();
});

// DELETE /configs/:name — remove a config
app.delete('/configs/:name', (req, res) => {
  stmts.remove.run(req.params.name);
  res.status(204).end();
});
...
app.get('/search', (req, res) => {
  const { tag, q, regex, fields, config: configPattern, limit: limitStr } = req.query;
  ...
  if (tag) {
    ...
    if (!check.safe) return res.status(400).json({ error: `Invalid regex: ${check.reason}` });
    let re;
    try { re = new RegExp(regexStr, 'i'); } catch { return res.status(400).json({ error: 'Invalid regex' }); }
    matcher = (id, link) => {
    ...
    return res.status(400).json({ error: 'Provide at least one of: tag, q, regex' });
  }
  ...
  if (configPattern) {
    ...
    if (!configCheck.safe) return res.status(400).json({ error: `Invalid config pattern: ${configCheck.reason}` });
  ...
  res.json({ results, configsSearched, linksScanned });
});
...
app.post('/cherry-pick', async (req, res) => {
  const { source, expression } = req.body || {};
  if (!source || !expression) {
    return res.status(400).json({ error: 'Provide "source" (config name) and "expression"' });
  }

  const row = stmts.get.get(source);
  if (!row) return res.status(404).json({ error: `Config "${source}" not found` });
  ...
  try {
    ...
    return res.status(501).json({ error: 'Run "pnpm build" from the repo root first — cherry-pick requires alap/core' });
  }
...
app.post('/query', async (req, res) => {
  const { expression, configName, configs: configNames } = req.body || {};
  if (!expression) {
    return res.status(400).json({ error: 'Provide "expression"' });
  }
  ...
  try {
    ...
    return res.status(501).json({ error: 'Run "pnpm build" from the repo root first — query requires alap/core' });
  }
  ...
  if (configNames && Array.isArray(configNames)) {
    ...
    if (loaded.length === 0) {
      return res.status(404).json({ error: 'None of the requested configs were found' });
    }
    config = mergeConfigs(...loaded);
  } else {
    const name = configName || 'demo';
    const row = stmts.get.get(name);
    if (!row) return res.status(404).json({ error: `Config "${name}" not found` });
    config = JSON.parse(row.config);
  }
...
app.listen(PORT, () => {
  console.log(`Alap config server running at http://localhost:${PORT}`);
