# Alap Config Server — Hono + SQLite

A minimal REST API server for persisting Alap configurations. Uses Hono and SQLite via `better-sqlite3`.
...
```bash
docker run -p 3000:3000 -v alap-data:/app/alap.db alap-server-hono
```
...
docker rmi alap-server-hono 2>/dev/null   # or: podman rmi alap-server-hono
```
