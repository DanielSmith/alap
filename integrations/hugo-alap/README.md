...
Hugo shortcodes are Go templates, not Go code. While Alap has a [full Go parser](../../src/other-languages/go/), Hugo's template engine can't call arbitrary Go functions. A custom Hugo build or a Go template reimplementation of the parser would be fragile and incomplete. The web component approach guarantees full feature access with zero maintenance burden.
