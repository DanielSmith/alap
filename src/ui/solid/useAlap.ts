...
export function useAlap(): UseAlapReturn {
  ...
  return {
    query: (expression, anchorId?) => engine.query(expression, anchorId),
    resolve: (expression, anchorId?) => engine.resolve(expression, anchorId),
    getLinks: (ids) => engine.getLinks(ids),
  };
}
