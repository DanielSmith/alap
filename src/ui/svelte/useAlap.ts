...
import { getAlapContext } from './context';
...
export function useAlap(): UseAlapReturn {
  const { engine } = getAlapContext();

  return {
    query: (expression: string, anchorId?: string) => engine.query(expression, anchorId),
    resolve: (expression: string, anchorId?: string) => engine.resolve(expression, anchorId),
    getLinks: (ids: string[]) => engine.getLinks(ids),
  };
}
