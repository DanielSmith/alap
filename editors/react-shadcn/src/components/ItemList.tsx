  ...
  return (
    <>
      ...
      <div className="px-4 py-1.5 text-xs flex justify-between items-center text-dim">
        ...
        <Button variant="ghost" size="sm" onClick={() => addItem()} className="text-xs text-accent h-auto px-1.5 py-0.5">
          + Add
        </Button>
      </div>
              ...
              {item.tags && item.tags.length > 0 && (
                <div className="flex gap-1 mt-1.5 flex-wrap">
                  {item.tags.map((tag) => (
                    <Badge key={tag}>
                      {tag}
                      <span className="tag-remove" onClick={(e) => handleRemoveTag(e, id, tag)}>x</span>
                    </Badge>
                  ))}
                </div>
              )}
