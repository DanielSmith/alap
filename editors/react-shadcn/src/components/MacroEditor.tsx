    ...
    <Collapsible open={expanded} onOpenChange={setExpanded}>
      <div className="rounded-xl fade-in edit-card" style={CARD_STYLE}>
        ...
        <CollapsibleTrigger asChild>
          <div className="flex items-center justify-between px-6 py-4 cursor-pointer select-none hover-bg-hover rounded-t-xl">
          ...
          <div className="px-6 pb-6">
            <div className="space-y-4 mb-4">
              <div>
                ...
                <Input mono value={editName} onChange={(e) => handleChange(setEditName, e.target.value)} onKeyDown={handleKeyDown} />
              </div>
              <div>
                <label className="block text-xs mb-1 font-semibold text-accent">Expression</label>
                <Input mono value={editExpr} onChange={(e) => handleChange(setEditExpr, e.target.value)} onKeyDown={handleKeyDown} />
              </div>
            </div>
            ...
            <div className="flex justify-between items-center pt-4" style={{ borderTop: '1px solid var(--alap-border-subtle)' }}>
              <Button variant="cancel" onClick={() => closeEditMacro(macroName)} className="text-sm px-6 py-2 rounded-2xl font-medium">
                Cancel
              </Button>
