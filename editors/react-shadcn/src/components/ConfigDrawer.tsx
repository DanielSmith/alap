      ...
      <div>
        ...
        <Select value={storageMode} onValueChange={(v) => setStorageMode(v as StorageMode)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="hybrid">Local + Remote</SelectItem>
          </SelectContent>
        </Select>
      </div>
...
function DrawerButton({ onClick, label, danger, Icon }: {
  ...
  return (
    <Button variant="outline" onClick={onClick}
      className="w-full justify-start text-sm px-3 py-2.5 rounded-lg hover-bg-input flex items-center gap-2.5 h-auto"
  ...
  return (
    <div className="flex gap-2 fade-in">
      <Input value={value} onChange={(e) => onChange(e.target.value)}
        ...
        placeholder={placeholder} className="flex-1 text-sm" autoFocus />
      <Button size="sm" onClick={onSubmit} style={SUBMIT_STYLE}>{submitLabel}</Button>
      <Button variant="ghost" size="sm" onClick={onCancel} className="text-muted">Cancel</Button>
    </div>
  );
}
