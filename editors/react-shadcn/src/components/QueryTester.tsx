  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {sorted.map(([tag, count]) => (
        <Badge key={tag} className="cursor-pointer" onClick={() => setTestQuery(`.${tag}`)}>
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {keys.map((key) => {
          ...
          return (
            <Badge key={key} variant="pattern" onClick={() => setTestQuery(`/${key}/`)}
              className="text-xs" style={PATTERN_BTN_STYLE} title={`/${pattern}/`}>
              /{key}/
            </Badge>
          );
        })}
