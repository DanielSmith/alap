...
**[Cookbook](README.md) › Obsidian:** [Overview](obsidian-overview.md) · [Get a Vault](obsidian-vault.md) · [Core Mode](obsidian-core.md) · **REST Setup** · [Hardening](obsidian-hardening.md)

Wiring Alap's `:obsidian:rest:` sub-mode to the **Obsidian Local REST API** plugin. This chapter is setup only — install, API key, self-signed cert, server config, and verification. For server-hardening guidance (which applies to both Core and REST), see [Hardening](obsidian-hardening.md).

If you haven't chosen between Core and REST yet, read [Overview — Core vs REST](obsidian-overview.md#core-vs-rest) first. If you just want to get data in, [Get a Vault](obsidian-vault.md) is upstream of this chapter.
...
See [Overview — General troubleshooting](obsidian-overview.md#general-troubleshooting) for issues that apply to both modes, and [Core Mode — Core-specific troubleshooting](obsidian-core.md#core-specific-troubleshooting) for filesystem-side checks.
...
This depends on your server hardening, not your REST setup. See [Hardening — The six defenses](obsidian-hardening.md#the-six-defenses-for-your-node-server), particularly host-header validation.
...
→ [Hardening](obsidian-hardening.md) — server-level defenses that apply to both Core and REST. If you're exposing this past a personal dev box, read it.
