...
**[Cookbook](README.md) › Obsidian:** [Overview](obsidian-overview.md) · **Get a Vault** · [Core Mode](obsidian-core.md) · [REST Setup](obsidian-rest-setup.md) · [Hardening](obsidian-hardening.md)
...
Skip to [Core Mode](obsidian-core.md) or [REST Setup](obsidian-rest-setup.md), depending on which sub-mode you want. Note the vault's absolute path on disk — you'll pass it to the server as `ALAP_OBSIDIAN_VAULT`.
...
→ [Core Mode](obsidian-core.md) if you want zero-plugin filesystem access (the usual starting point).
→ [REST Setup](obsidian-rest-setup.md) if you've decided on REST for fuzzy search.
→ [Overview — Core vs REST](obsidian-overview.md#core-vs-rest) if you're still deciding.
