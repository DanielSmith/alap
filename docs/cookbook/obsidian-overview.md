...
**[Cookbook](README.md) › Obsidian:** **Overview** · [Get a Vault](obsidian-vault.md) · [Core Mode](obsidian-core.md) · [REST Setup](obsidian-rest-setup.md) · [Hardening](obsidian-hardening.md)
...
> Alap is a single-maintainer open source project that hasn't been through a third-party audit. Please do your own due diligence — especially when wiring up protocols on servers with local network access. See the [Hardening](obsidian-hardening.md) chapter for the server-side checklist.
...
2. **[Get a Vault](obsidian-vault.md)** — already have one · convert markdown · convert Hugo/Jekyll/MkDocs/Docusaurus · convert RSS/Atom
3. **[Core Mode](obsidian-core.md)** — zero plugin, filesystem only; inline tags; tag aliases; core troubleshooting
4. **[REST Setup](obsidian-rest-setup.md)** — Local REST API plugin install · API key · self-signed cert · REST troubleshooting
5. **[Hardening](obsidian-hardening.md)** — library safeguards · six-defense server checklist · auditing an install

## Prerequisites

- **Obsidian** — install from [obsidian.md](https://obsidian.md). Free for personal use.
- **Node 22+** and **pnpm** — `corepack enable && corepack prepare pnpm@latest --activate` on a fresh machine.
- **Python 3.9+** — only if you're converting content ([Get a Vault](obsidian-vault.md) paths B–D). Included on macOS and every Linux distro.
- **An Alap clone** — `git clone https://github.com/DanielSmith2017/alap && cd alap && pnpm install`.

No Obsidian plugin is required for Core mode. REST mode needs one — walked through in [REST Setup](obsidian-rest-setup.md).
...
| `tagAliases` | `Record<string,string>` | `{}` | Bridge Alap-safe handles to raw Obsidian tag strings. See [Core Mode — Tag aliases](obsidian-core.md#tag-aliases). |
...
Mode-specific troubleshooting lives with each mode chapter — see [Core Mode](obsidian-core.md#core-specific-troubleshooting) and [REST Setup](obsidian-rest-setup.md#rest-specific-troubleshooting). This section collects issues that apply regardless of which sub-mode you're running.
...
- [Core Concepts — Protocols](../core-concepts/protocols.md#obsidiancore--search-a-local-obsidian-vault)
- [examples/sites/obsidian/](../../examples/sites/obsidian/) — minimal Core-only reference
- [Threat model](../security/threat-model.md) — orientation on what the library does, doesn't do, and assumes from integrators
- [Security reference](../api-reference/security.md) — full per-feature security model
